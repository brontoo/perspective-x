...
const SCENE_VISUALS = {
    ...
    gas_boyle_adnoc: [
        () => (
            <div className="flex flex-col items-center gap-4 w-full">
                <div className="flex items-end gap-6 justify-center">
                    <div className="text-5xl">🏭</div>
                    <div className="flex flex-col items-center gap-1">
                        <div className="text-xs text-slate-400">ADNOC Storage Sphere</div>
                        <motion.div animate={{ scale: [1, 1.03, 1] }} transition={{ duration: 2, repeat: Infinity }} className="w-20 h-20 rounded-full border-2 border-cyan-500/50 bg-cyan-500/10" />
                    </div>
                    <div className="text-4xl">🧯</div>
                </div>
                <div className="text-xs text-cyan-300">Pressure control operation in progress</div>
            </div>
        ),
        () => (
            <div className="flex flex-col items-center gap-4 w-full">
                <div className="text-xs text-slate-400">Boyle's Law Particle Compression</div>
                <div className="relative w-64 h-36 rounded-xl border border-cyan-500/40 bg-slate-900/50 overflow-hidden">
                    {Array.from({ length: 18 }).map((_, i) => (
                        <motion.div
                            key={i}
                            className="absolute w-2 h-2 rounded-full bg-cyan-300"
                            animate={{
                                x: [10 + (i % 6) * 40, 20 + (i % 4) * 28, 10 + (i % 6) * 40],
                                y: [10 + Math.floor(i / 6) * 30, 8 + Math.floor(i / 5) * 24, 10 + Math.floor(i / 6) * 30]
                            }}
                            transition={{ duration: 1.2 + (i % 3) * 0.2, repeat: Infinity, ease: 'linear' }}
                        />
                    ))}
                    <motion.div
                        className="absolute top-0 right-0 h-full border-l-2 border-amber-400/70 bg-amber-500/10"
                        animate={{ width: ['40%', '18%', '40%'] }}
                        transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
                    />
                </div>
                <div className="text-xs text-amber-300">Volume decreases → pressure increases</div>
            </div>
        ),
        null,
        null,
        () => (
            <div className="flex flex-col items-center gap-4 w-full">
                <motion.div
                    animate={{ boxShadow: ['0 0 0 rgba(34,211,238,0.15)', '0 0 30px rgba(34,211,238,0.35)', '0 0 0 rgba(34,211,238,0.15)'] }}
                    transition={{ duration: 1.4, repeat: Infinity }}
                    className="px-5 py-3 rounded-xl border border-emerald-500/40 bg-emerald-500/10"
                >
                    <p className="text-emerald-300 text-sm font-semibold">Safe Setpoint Confirmed: 200 kPa</p>
                </motion.div>
                <div className="text-xs text-slate-400">Compression cleared by safety protocol</div>
            </div>
        )
    ],
    ...
    gas_charles_aviation: [
        () => (
            <div className="flex flex-col items-center gap-4 w-full">
                <div className="flex items-center gap-6">
                    ...
                    <motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 1.2, repeat: Infinity }} className="text-3xl">☀️</motion.div>
                    <div className="text-4xl">👷‍♀️</div>
                </div>
                <div className="text-xs text-amber-300">Abu Dhabi runway heat stress conditions</div>
            </div>
        ),
        () => (
            <div className="flex flex-col items-center gap-4 w-full">
                <div className="text-xs text-slate-400">Charles's Law Particle Expansion</div>
                <div className="relative w-64 h-36 rounded-xl border border-orange-500/40 bg-slate-900/50 overflow-hidden">
                    {Array.from({ length: 14 }).map((_, i) => (
                        <motion.div
                            key={i}
                            className="absolute w-2 h-2 rounded-full bg-orange-300"
                            animate={{
                                x: [20 + (i % 4) * 24, 10 + (i % 5) * 42, 20 + (i % 4) * 24],
                                y: [20 + Math.floor(i / 4) * 24, 10 + Math.floor(i / 3) * 30, 20 + Math.floor(i / 4) * 24]
                            }}
                            transition={{ duration: 1 + (i % 4) * 0.15, repeat: Infinity, ease: 'easeInOut' }}
                        />
                    ))}
                    <motion.div
                        className="absolute inset-0 border-2 border-yellow-400/60 rounded-xl"
                        animate={{ scale: [0.84, 1, 0.84] }}
                        transition={{ duration: 2, repeat: Infinity }}
                    />
                </div>
                <div className="text-xs text-yellow-300">Temperature increases → volume increases</div>
            </div>
        ),
        null,
        null,
        () => (
            <div className="flex flex-col items-center gap-4 w-full">
                <div className="px-5 py-3 rounded-xl border border-cyan-500/40 bg-cyan-500/10">
                    <p className="text-cyan-300 text-sm font-semibold">Expansion target validated: 6.0 L</p>
                </div>
                <div className="text-xs text-slate-400">Ground support systems remain within tolerance</div>
            </div>
        )
    ],
    ...
    gas_gaylussac_cylinder: [
        () => (
            <div className="flex flex-col items-center gap-4 w-full">
                <div className="flex items-center gap-6">
                    <div className="text-5xl">🛢️</div>
                    <motion.div animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 1, repeat: Infinity }} className="text-2xl">🔥</motion.div>
                    <div className="text-4xl">🏭</div>
                </div>
                <div className="text-xs text-red-300">Sealed cylinder near heated process line</div>
            </div>
        ),
        () => (
            <div className="flex flex-col items-center gap-4 w-full">
                <div className="text-xs text-slate-400">Gay-Lussac Pressure Rise Animation</div>
                <div className="relative w-64 h-36 rounded-xl border border-red-500/40 bg-slate-900/50 overflow-hidden">
                    {Array.from({ length: 16 }).map((_, i) => (
                        <motion.div
                            key={i}
                            className="absolute w-2 h-2 rounded-full bg-red-300"
                            animate={{
                                x: [18 + (i % 5) * 30, 18 + (i % 5) * 30 + (i % 2 ? 8 : -8), 18 + (i % 5) * 30],
                                y: [14 + Math.floor(i / 5) * 26, 14 + Math.floor(i / 5) * 26 + (i % 2 ? -8 : 8), 14 + Math.floor(i / 5) * 26]
                            }}
                            transition={{ duration: 0.5 + (i % 3) * 0.08, repeat: Infinity, ease: 'linear' }}
                        />
                    ))}
                    <motion.div
                        className="absolute bottom-2 left-2 right-2 h-2 rounded bg-red-500/40"
                        animate={{ opacity: [0.3, 1, 0.3] }}
                        transition={{ duration: 0.8, repeat: Infinity }}
                    />
                </div>
                <div className="text-xs text-rose-300">Temperature increases → pressure increases</div>
            </div>
        ),
        null,
        null,
        () => (
            <div className="flex flex-col items-center gap-4 w-full">
                <motion.div
                    ...
                    className="px-5 py-3 rounded-xl border border-emerald-500/40 bg-emerald-500/10"
                >
                    <p className="text-emerald-300 text-sm font-semibold">Risk controlled at 225 kPa</p>
                </motion.div>
                <div className="text-xs text-slate-400">Cooling and isolation completed safely</div>
            </div>
        )
    ],
